...
本書は、シフト自動作成デモのシステム構成、モジュールの責務、主要な処理フロー、データ保存方法、エラー処理方針を説明する。
...
- 画面表示、業務処理、データアクセスを分離する
- SQLをRepository層へ集約する
- 入力検証、シフト生成、生成後検証を分離する
- 自動生成後の手動変更にも同じ制約検証を適用する
- 同じ配置データから複数の表示・出力形式を生成する
- 月間シフトの保存はトランザクションで行う
- 共通する画面処理を再利用する
- 各処理を自動テストしやすい構造にする

Streamlitページには画面制御を中心に記述し、業務ルール、DB操作、表示データ変換は専用モジュールへ分離する。
...
希望休の登録・削除では、対象年月、従業員、処理種別ごとに選択状態を分離して保持する。

シフト表示では、保存済みの同一配置データを日付別表と従業員別表へ変換して表示する。

画面モジュールではSQLを直接実行しない。
...
`export_service.py`は、保存済みシフトを管理用・個人配布用の複数形式へ変換する。
...
希望休の登録・削除は画面上で複数日を選択できるが、業務処理では既存の1日単位の登録・削除処理を日付ごとに呼び出す。

登録済みの日付は新規登録対象にできず、未登録の日付は削除対象にできない。
...
月間シフトは次の2形式をタブで切り替えて表示する。

- 日付別：各日の早番・遅番配置者を確認する
- 従業員別：各従業員の月間勤務を確認する
...
CSVは書式情報を保持しないため、背景色、罫線、列幅などは出力しない。
