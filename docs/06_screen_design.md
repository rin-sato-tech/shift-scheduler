...
- 複数日の選択や表示切替を直感的に操作できるようにする
- 色だけに依存せず、文字でも状態を識別できるようにする
- DB更新前に入力値を検証する
- 破壊的な操作や上書き操作には注意または確認を設ける
...
| SC-02  | 希望休入力     | `pages/2_希望休入力.py`     | カレンダーによる希望休の登録・削除   |
| SC-03  | 必要人数設定   | `pages/3_必要人数設定.py`   | 日付・シフトごとの必要人数設定       |
| SC-04  | シフト生成     | `pages/4_シフト生成.py`     | 自動生成と日付別・従業員別の結果確認 |
| SC-05  | シフト手動変更 | `pages/5_シフト手動変更.py` | 保存済みシフトの変更・再検証         |
| SC-06  | CSV出力        | `pages/6_CSV出力.py`        | 管理用・個人配布用のCSV出力          |
...
希望休入力画面では、対象月の日付を月間カレンダー形式で表示する。
...
日付はボタンとして表示し、クリックによって選択・解除できるようにする。

状態は、背景色または表示記号と文字によって識別できるようにする。
...
対象月の従業員別希望休を、月間カレンダーから登録・確認・削除する。
...
有効従業員が0件の場合は登録領域を表示せず、従業員管理画面で登録するよう案内する。

#### 登録用カレンダー

対象月の日付を月曜日始まりのカレンダーで表示する。
...
| -------- | ---------------------------------- |
| 未選択   | 登録対象として選択されていない     |
| 選択中   | 今回の登録対象として選択されている |
| 登録済み | すでに希望休として保存されている   |
| 対象月外 | 空欄または操作不可                 |

操作規則：

- 未登録の日付をクリックすると選択状態になる
- 選択中の日付を再度クリックすると選択解除される
- 登録済みの日付は新規登録対象として選択できない
- 複数の日付を同時に選択できる

#### 選択内容

登録対象として選択中の日付を、カレンダー下部に一覧表示する。

選択がない場合は、日付を選択するよう案内する。
...
- 登録成功件数をFlashメッセージで表示する
- 対象従業員の登録用選択状態を解除する
- 希望休一覧と集計を再取得する
- 画面を再描画する

一部の日付で登録に失敗した場合は、成功件数と失敗理由を表示する。
...
希望休が1件以上登録されている従業員から、削除対象者を選択する。
...
対象月に希望休がない場合は削除領域を表示しない。

#### 削除用カレンダー

選択した従業員の登録済み希望休をカレンダー形式で表示する。

日付の状態は次のように区別する。

| 状態       | 内容                               |
| ---------- | ---------------------------------- |
| 未登録     | 希望休が登録されておらず操作不可   |
| 登録済み   | 希望休として保存されている         |
| 削除選択中 | 今回の削除対象として選択されている |
| 対象月外   | 空欄または操作不可                 |

操作規則：

- 登録済みの日付をクリックすると削除選択状態になる
- 削除選択中の日付を再度クリックすると選択解除される
...
削除対象が0件の場合はボタンを無効化する。

#### 処理後

- 削除成功件数をFlashメッセージで表示する
- 対象従業員の削除用選択状態を解除する
- 希望休一覧と集計を再取得する
- 画面を再描画する
...
選択状態は、少なくとも次の条件ごとに区別する。

- 対象年月
- 従業員ID
- 登録・削除の処理種別

対象月または従業員を変更した場合、別条件の選択状態を現在のカレンダーへ表示しない。

登録・削除の完了後は、該当する選択状態を解除する。
...
生成結果は、日付単位と従業員単位の2形式で確認できるようにする。
...
同一日付・同一シフトに複数人が配置されている場合は、同一セル内に表示する。

手動変更された配置者には`※`を付ける。

#### 従業員別月間シフト表

従業員ごとの月間勤務を確認するための表とする。
...
対象日の配置が存在しない場合は、表示上の`休`として補完する。

#### 色分け

従業員別月間シフト表は、勤務区分に応じて日付セルの背景色を変更する。
...
色は識別を補助するものであり、セル内の`早`、`遅`、`休`の文字を残す。

従業員ID・氏名などの日付以外の列には、勤務区分の背景色を適用しない。

表の横幅が画面を超える場合は、横スクロールによって確認できるようにする。
...
検証結果は、日付別表と従業員別表の両方に共通するものとする。

表示形式の切替によって、保存データや検証結果を変更しない。
...
保存済みの月間シフトを、管理用および従業員への配布用CSVとしてダウンロードする。
...
```text
shift_employee_monthly_YYYYMM.csv
```
...
選択した従業員1名の月間勤務を、従業員へ配布しやすいカレンダー形式で出力する。
...
```text
shift_calendar_従業員ID_YYYYMM.csv
```
...
| 希望休入力     | `day_off_service.py`、`ui_helpers.py`            |
...
| v1.1.0     | 2026-08-04 | 希望休カレンダー、従業員別表示、CSV出力を追加 |
