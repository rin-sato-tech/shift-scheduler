...
v1.1.0では、希望休入力、シフト表示、CSV出力を改善したが、シフト生成の制約、目的関数、Solverモデルは変更していない。
...
flowchart TD
    ...
    N --> O[日付別・従業員別のシフトと集計を表示]
```

生成処理は、Solverが解を返しただけでは成功としない。

生成された配置を共通の検証処理に通し、ハード制約違反がないことを確認してから保存する。

保存後の表示形式やCSV形式は、保存された配置データから別途生成する。
...
v1.1.0では、希望休を月間カレンダーから複数選択して登録・削除できる。

この変更は画面操作の改善であり、制約モデル上の希望休の扱いは変更しない。

希望休は引き続き次の組み合わせで管理する。

```text
従業員ID + 対象日
```

複数日を一括登録した場合も、DB上では1日につき1件の希望休として保存する。

Solverでは、希望休に登録された日の早番・遅番を勤務不可とする。
...
従業員別月間シフト表と個人用カレンダーでは、配置が存在しない日を`休`または`休み`として表示する。

この表示値は、Solverのシフト区分やDB上のレコードではない。

シフト区分の集合は引き続き次の2種類とする。

$$
S = {\mathrm{early},\ \mathrm{late}}
$$

従業員`e`、日付`d`について、すべてのシフト配置変数が0である場合に、表示変換処理が休みとして扱う。

$$
\sum_{s \in S}x_{e,d,s}=0
$$
...
探索時間内に実行可能解が見つかった場合は`FEASIBLE`、最適性まで証明された場合は`OPTIMAL`となる。

探索時間内に結果を確定できなかった場合は`UNKNOWN`となる可能性がある。
...
日付別表、従業員別表、個人用カレンダーは、同じ保存済み配置を異なる形式へ変換したものである。

表示形式やCSV形式によって、次の内容は変化しない。
...
背景色は画面表示上の補助情報であり、制約判定には使用しない。

CSV出力は保存済み配置を読み取るだけであり、シフト生成や検証結果を変更しない。
...
| v1.1.0     | 2026-08-04 | 制約変更なし。希望休UI、表示、CSV出力との関係を追記 |
