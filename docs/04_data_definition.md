...
- 表示・CSV出力専用の表構造はDBへ保存せず、実行時に生成する
...
v1.1.0ではテーブルおよびカラムの追加・変更を行わない。

従業員別月間シフト表と個人用シフトカレンダーは、`employees`と`schedules`をもとに実行時に生成する。
...
### `schedule_generations`の位置づけ

`schedule_generations`は、自動生成処理の結果を保存する。
...
`schedules.generation_id`により、自動生成された配置と生成結果を関連付ける。

現在の画面では生成履歴の一覧・詳細を表示しないが、生成処理と保存配置の関連を保持するためにテーブルを使用する。

手動変更後に保存された配置では、元の`generation_id`を保持する場合と`NULL`になる場合があるため、`generation_id`はNULL許可とする。
...
v1.1.0で追加した次のデータは、独立したDBテーブルとして保存しない。
...
これらは、保存済みの`employees`および`schedules`から実行時に生成する。

### 従業員別月間シフト表

行を従業員、列を対象月の日付とする表示・出力用DataFrameである。
...
対象日について`schedules`のレコードが存在しない場合に、表示変換処理で補完する。

### 個人用シフトカレンダー

選択した従業員1名の`schedules`を、月曜日始まりの月間カレンダーへ変換する。

対象月外の日付に相当するセルは空欄とする。

勤務のない日は、表示変換時に`休み`として補完する。

### 色分け情報

早番・遅番・休みの背景色は画面表示上の情報であり、DBには保存しない。

色分けは勤務区分の文字列をもとにStreamlit表示時に適用する。
...
| バージョン | 日付       | 内容                                                |
| ---------- | ---------- | --------------------------------------------------- |
| v1.0.0     | 2026-08-02 | 初版                                                |
| v1.1.0     | 2026-08-04 | DB構造に変更なし。表示・CSV用データの生成方針を追記 |
